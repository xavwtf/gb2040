#pragma once

#include "cpu.h"
#include "mmu.h"
#include "mbc.h"
#include "timer.h"
#include "graphics.h"
#include "ppu.h"
#include "apu.h"
#include "../platform/platform.h"

#include <cstdint>
#include <string>
#include <functional>
#include <memory>

#define CYCLES_PER_FRAME 70224
#define GB_CLOCK_SPEED 4194304
#define GB_FRAME_TIME_US 1e6 / (GB_CLOCK_SPEED * 1) * CYCLES_PER_FRAME

namespace GB2040::Core
{

using GB2040::Platform::Platform;
using GB2040::Platform::ROMSource;

enum class GBMode { DMG, CGB };

enum class Interrupt {
    VBLANK,
    STAT,
    TIMER,
    SERIAL,
    JOYPAD
};

struct CartridgeHeader {
    uint8_t logoData[0x30];
    char title[0xF];
    uint8_t cgbFlag; // $80 = backwards compatible, $C0 = no backwards compatibility
    uint16_t newLicenseeCode;
    uint8_t sgbFlag;
    CartType cartType;
    uint8_t romSize; // 32 KiB * (1 << romSize)
    uint8_t ramSize; // https://gbdev.io/pandocs/The_Cartridge_Header.html#0149--ram-size
    uint8_t region; // set if only available overseas (and not japan)
    uint8_t oldLicenseeCode;
    uint8_t versionNo; // unused
    uint8_t headerChecksum;
    uint16_t globalChecksum; // unused, not checked by boot rom
} __attribute__((packed));

enum class Button : uint8_t {
    A,
    B,
    SELECT,
    START,
    RIGHT,
    LEFT,
    UP,
    DOWN
};

class Console {

public:
    Console(Platform*, ROMSource*);
    ~Console(void);

    Platform* platform;

    GBMode mode;
    CartridgeHeader header;

    std::unique_ptr<CPU> cpuPtr;
    std::unique_ptr<MMU> mmuPtr;
    std::unique_ptr<IMBC> mbc = nullptr; // initialised dynamically so no reference
    std::unique_ptr<Timer> timerPtr;
    std::unique_ptr<PPU> ppuPtr;
    std::unique_ptr<APU> apuPtr;

    CPU& cpu;
    MMU& mmu;
    Timer& timer;
    PPU& ppu;
    APU& apu;

    uint8_t input;
    bool inputSelectButtons;
    bool inputSelectDpad;

    bool running;

    static constexpr uint8_t bootRom[0x100] { // i reimplemented it guys trust
        
    };

    size_t tick(void);
    size_t doTicks(size_t);
    void run(void);
    void requestInterrupt(Interrupt);
    void pressButton(Button);
    void releaseButton(Button);
    uint8_t getInputRegister(void);
    void save(void);
};

} // namespace GB2040::Core